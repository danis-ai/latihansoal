...
// Jika mata pelajaran tidak dipilih, arahkan kembali ke halaman pemilihan
if (!mataPelajaran) {
    alert('Mata pelajaran tidak dipilih.');
    ...
    if (answeredQuestions[currentQuestionIndex]) {
        ...
        allChoices.forEach(button => {
            if (button.textContent === correctAnswer) {
                button.style.backgroundColor = '#27ae60'; // Hijau untuk jawaban benar
                button.style.color = '#fff';
            }
            if (button.textContent === userAnswer && userAnswer !== correctAnswer) {
                button.style.backgroundColor = '#c0392b'; // Merah untuk jawaban salah
                button.style.color = '#fff';
            }
            // Nonaktifkan semua pilihan setelah dijawab
